...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, causal_mask: bool = False) -> torch.Tensor:
        ...
        # (Batch_Size, Seq_Len, Dim) -> (Batch_Size, Seq_Len, Head, Dim / H) -> (Batch_Size, Head, Seq_Len, Dim / H)
        ...
        if causal_mask: 
            # Mask where the upper triangle (above the principla diagonal) is made up of 1
            mask = torch.ones_liek(weight, dtype=torch.bool).triu(1)
        ...
        # (Batch_Size, Head, Seq_Len, Seq_len) -> (Batch_Size, H, Seq_Len, Dim / H) -> (Batch_Size, H, Seq_Len, Dim / H)
        output = weight @ v
...
class CrossAttention(nn.Module): 
    ...
    def forward(self, x, y): 
        ...
        intermediate_shape = (batch_size, -1, n_head, self.d_head)
